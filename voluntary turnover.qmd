...
output_name <- paste0("outputs/Voluntary Turnover Rate Monthly ", start_date, " to ", end_date, ".xlsx")
...
for (sheet_name in names(monthly_turnover_data)) {
  ...
  addStyle(wb, sheet = sheet_name, style = createStyle(numFmt = "0%"), cols = voluntary_turnover_col, 
          rows = 2:(nrow(monthly_turnover_data[[sheet_name]]) + 1), gridExpand = TRUE)
  addStyle(wb, sheet = sheet_name, style = createStyle(numFmt = "0%"), cols = ytd_annualized_col, 
          rows = 2:(nrow(monthly_turnover_data[[sheet_name]]) + 1), gridExpand = TRUE)
}
