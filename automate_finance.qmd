...
```{r}
# split by segment
ifr_segment_split <- split(ifr_segment, ifr_segment$segment_function)

ifr_division_split <- split(ifr_division, ifr_division$segment_function)

ifr_plant_split <- split(ifr_plant, ifr_plant$segment_function)
```
...
voluntary_turnover_bur <- function(previous_output = vt_prev, current_output = vt_current, attribute = 'Segment') {
      ...
      current <- previous_output[[attribute]]  |>
        mutate(data_source = "CY Actual") |> 
        select(-headcount, -voluntary_terminations, -ytd_annualized) |> 
        pivot_wider(names_from = month, values_from = voluntary_turnover) |> 
        select(where(is.character), "ytd", all_of(column_order_dynamic))
...
```{r}
# split by segment
vt_segment_split <- split(vt_segment, vt_segment$segment_function)

vt_division_split <- split(vt_division, vt_division$segment_function)

vt_plant_split <- split(vt_plant, vt_plant$segment_function)
```
