...
# remove duplicate row for promotions - both inbound and outbound
...
for (sheet_name in names(results_named)) {
  ...
  internal_fill_rate_col <- which(names(results_named[[sheet_name]]) == "Internal Fill Rate")
  ...
  if (length(internal_fill_rate_col) > 0) {
    addStyle(wb, sheet = sheet_name, 
             ...
             rows = 2:(nrow(results_named[[sheet_name]]) + 1), 
             gridExpand = TRUE)
  }
}
...
#     "Promo or Lateral No New Profile" = qa_change_dates_same |> 
#          filter(str_detect(business_process_reason, "Promotion") | 
#                              str_detect(business_process_reason, "Lateral"))
#     ), 
