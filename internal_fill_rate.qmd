...
df_worker_changes_raw <- readxl::read_xlsx(file_path_worker_changes, skip = 7) |> 
  janitor::clean_names() |> 
  rename(prior_segment_assignment = previous_segment_assignment)
...
qa_multiple_moves <- full_df_no_exceptions %>%
...
```{r}
# MANUAL CORRECTION
# this was supposed to be a realignment but positions were changed
full_df <- full_df_no_exceptions |> 
  filter(!(division_function == "IPS Couplings Division" & 
          business_process_type == "Edit Position")) |> 
  filter(is.na(prior_job_profile) | prior_job_profile != "Title Mapping in Progress (DL)")
```
...
full_df_scrubbed <- full_df |> 
  select(employee_id, worker, hire_date, time_in_job_profile_start_date, internal_external, employee_type,
         ...
         prior_job_profile, prior_career_level, prior_comp_grade, prior_segment_assignment,
         new_job_profile, new_job_family, new_career_level, new_comp_grade, current_segment_assignment) |> 
